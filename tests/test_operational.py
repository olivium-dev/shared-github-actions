...
class OperationalContractTests(unittest.TestCase):
    ...
    def test_guest_validates_every_seed_login_and_gateway_currency_wallet(self) -> None:
        seed = seed_data()
        roster = {
            "users": [
                {
    ...
    def test_environment_rewrites_stage_dependencies_to_the_lease(self) -> None:
        ...
        self.assertEqual("1", result["PartnerWallet__CurrencyId"])
    ...
    def test_nginx_routes_cms_and_gateway_traffic_without_exposing_private_ports(self) -> None:
        ...
        self.assertIn("location ~ ^/(?:api|v1|admin|health|transcribe)(?:/|$)", config)
        self.assertLess(config.index("|transcribe)"), config.index("proxy_pass http://127.0.0.1:10000;"))
